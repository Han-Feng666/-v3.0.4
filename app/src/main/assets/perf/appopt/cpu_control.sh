...
local msm_irqbalance_service="${1}"
local found=0
for prefix in system vendor odm product system_ext my_product
do
	if [ "$(getprop init.svc.${prefix}.${msm_irqbalance_service})" != "" ] || [ "$(getprop init.svc.${msm_irqbalance_service})" != "" ] ; then
		found=1
		if [ -n "$(getprop init.svc.${prefix}.${msm_irqbalance_service})" ] && [ "$(getprop init.svc.${prefix}.${msm_irqbalance_service})" != "stopped" ] ;then
			setprop ctl.stop ${prefix}.${msm_irqbalance_service} >/dev/null 2>&1
			setprop init.svc.${prefix}.${msm_irqbalance_service} stopped >/dev/null 2>&1
			stop ${prefix}.${msm_irqbalance_service} >/dev/null 2>&1
		fi
		if [ -n "$(getprop init.svc.${msm_irqbalance_service})" ] && [ "$(getprop init.svc.${msm_irqbalance_service})" != "stopped" ] ;then
...
if [ "$found" = "0" ] ;then
	echo "- 未找到${1}服务……"
elif [ -z "$(pgrep $msm_irqbalance_service 2>/dev/null)" ] && [ "$found" != "0" ] ;then
	echo "- 服务${msm_irqbalance_service}已禁用……"
else
	echo "- 服务${msm_irqbalance_service}仍顽固存活……"
fi
}
...
/dev/cpuset/rt/cpus
"
local HyperOS_cpus_file="
/dev/cpuset/foreground/cpus
"
local max_cpus=$(cat /sys/devices/system/cpu/present 2>/dev/null )
local architect=$(out=; for f in /sys/devices/system/cpu/cpufreq/*/related_cpus; do [ -f "$f" ] || continue; read -r cpus < "$f"; count=0; for token in $(echo "$cpus" | tr ',' ' '); do case $token in *-*) count=$((count + ${token#*-} - ${token%-*} + 1)) ;; *) count=$((count + 1)) ;; esac; done; out="${out}+${count}"; done; echo "${out#+}")
local HyperOS_Foreground_cpus
local HyperOS_version=`getprop | grep -Ei ".*mi.*os.*version.*name"`
[ -z "${HyperOS_version}" ] && HyperOS_version=`getprop ro.mi.os.version.code`
case "${architect}" in
4+3+1|4+4) HyperOS_Foreground_cpus="2-7" ;;
3+4+1) HyperOS_Foreground_cpus="3-7" ;;
2+3+2+1) HyperOS_Foreground_cpus="2-7" ;;
6+2) HyperOS_Foreground_cpus="1-7" ;;
*) HyperOS_Foreground_cpus="$max_cpus" ;;
esac
...
*[Oo][Ss]*[234].[0-9]*|[234])
	for cpus in ${HyperOS_cpus_file}
	do
		lock_value "${HyperOS_Foreground_cpus}" "${cpus}"  
	done
;;
esac
