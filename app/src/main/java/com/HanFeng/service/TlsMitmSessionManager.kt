...
object TlsMitmSessionManager {
    ...
    private fun bypassCooldownMillisForReason(reason: String): Long {
        ...
        return when {
            normalized.startsWith("bridge-executor-saturated:") -> 5 * 1000L
            normalized.startsWith("ssl-pinning:") -> 20 * 60 * 1000L
            normalized.startsWith("ssl-trust-anchor:") || normalized.startsWith("ssl-hostname:") -> 15 * 60 * 1000L
            normalized.startsWith("ssl-certificate-time:") -> 10 * 60 * 1000L
            normalized.startsWith("ssl-protocol:") -> 5 * 60 * 1000L
            normalized.startsWith("ssl-handshake:") && listOf("certificate", "cert", "pin", "trust anchor", "hostname").any { normalized.contains(it) } -> 15 * 60 * 1000L
            normalized.startsWith("ssl-handshake:") -> 30 * 1000L
            normalized.startsWith("io-bridge:") && listOf("connection reset", "broken pipe", "eof").any { normalized.contains(it) } -> 10 * 1000L
            normalized.startsWith("io-bridge:") -> 5 * 1000L
            else -> 3 * 60 * 1000L
        }
    }
