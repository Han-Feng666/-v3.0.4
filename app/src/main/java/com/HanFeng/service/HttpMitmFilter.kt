...
(function(){
    try {
        ...
        var cleanupAdNodes = function(root) {
            try {
                root = root || document;
                if(!root || !root.querySelectorAll) return;
                root.querySelectorAll('iframe,script,img,video,source,link,a').forEach(function(el){
                    var url = el.src || el.href || (el.getAttribute && (el.getAttribute('src') || el.getAttribute('href') || el.getAttribute('data-src') || el.getAttribute('data-url')));
                    if(isAdLikeUrl(url)) el.remove && el.remove();
                });
                root.querySelectorAll('[class*="ad"],[id*="ad"],[class*="banner"],[id*="banner"],[class*="promo"],[class*="sponsor"],[class*="commercial"],[class*="splash"],[class*="interstitial"],[class*="reward"],[data-ad],[data-ad-slot],[data-ad-unit],[data-google-query-id],[data-promotion],[data-campaign],[data-deeplink],[data-market],[data-download]').forEach(function(el){
                    var text = '';
                    try { text = (el.className || '') + ' ' + (el.id || '') + ' ' + (el.innerText || ''); } catch(e) {}
                    if(isAdLikeText(text) || /(^|[-_\s])ad($|[-_\s])|banner|promo|sponsor|commercial|splash|interstitial|reward/i.test(text)) el.remove && el.remove();
                });
            } catch(e) {}
        };
        cleanupAdNodes(document);
        var cleanupTimer = window.setInterval ? window.setInterval(function(){ cleanupAdNodes(document); }, 1200) : null;
        if(window.IntersectionObserver) {
            var OriginalIntersectionObserver = window.IntersectionObserver;
            window.IntersectionObserver = function(callback, options) {
                var wrapped = function(entries, observer) {
                    try {
                        entries = Array.prototype.filter.call(entries || [], function(entry){
                            var target = entry && entry.target;
                            var text = target ? ((target.className || '') + ' ' + (target.id || '')) : '';
                            return !isAdLikeText(text);
                        });
                    } catch(e) {}
                    return callback.call(this, entries, observer);
                };
                return new OriginalIntersectionObserver(wrapped, options);
            };
        ...
        if(window.ResizeObserver) {
            ...
            window.ResizeObserver = function(callback) {
                ...
                return new OriginalResizeObserver(wrapped);
            };
